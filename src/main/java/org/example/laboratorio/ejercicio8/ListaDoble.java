...
public class ListaDoble<T> implements Iterable<T> {
	...
	public ListaDoble<Persona> obtenerPersonasCedulaPar() {
		ListaDoble<Persona> personasCedulaPar = new ListaDoble<>();
		NodoDoble<Persona> actual = (NodoDoble<Persona>) nodoPrimero;

		while (actual != null) {
			String cedula = actual.getValorNodo().getNumeroIdentificacion();
			if (cedula.length() % 2 == 0) {
				personasCedulaPar.agregarfinal(actual.getValorNodo());
			}
